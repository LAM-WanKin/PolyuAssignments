...
def keypoint_match(img1, img2, max_n_match=100, draw=True):
    ...
    if draw:
        ...
        img_keypoint1 = cv2.drawKeypoints(img1, keypoints1, None, cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        img_keypoint2 = cv2.drawKeypoints(img2, keypoints2, None, cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        write_and_show('results/1.3_img1_keypoints.jpg', img_keypoint1)
        write_and_show('results/1.3_img2_keypoints.jpg', img_keypoint2)

    # TODO: Knn match and Lowe's ratio test
    matcher = cv2.FlannBasedMatcher_create()
    best_2 = matcher.knnMatch(queryDescriptors=descriptors1,
...
def draw_match(img1, keypoints1, img2, keypoints2, match, savename):
    ...
    match_draw = cv2.drawMatches(img1=img1,
...
def transform(img, img_kps, dst_kps, H, W):
    ...
    T, status = cv2.findHomography(img_kps,
                                   dst_kps,
                                   cv2.USAC_ACCURATE,
                                   3)
    ...
    transformed = cv2.warpPerspective(img,
                                      T,
                                      (W, H),
...
if __name__ == '__main__':
    ...
    # write_and_show('results/new_img1.jpg', new_img1)
